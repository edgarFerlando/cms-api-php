    ...
    <div class="box">
        ...
        <div class="box-body">
            ...
            <div class="form-group {!! $errors->has('branch_code') ? 'has-error' : '' !!}">
                {!! Form::label('branch_code', trans('app.branch_code').' *') !!}
                {!! Form::text('branch_code', val($taxoMeta, 'branch_code'), [ 'class' => 'form-control slug-source', 'autocomplete' => 'off' ]) !!}
                @if ($errors->first('branch_code'))
                    <span class="help-block">{!! $errors->first('branch_code') !!}</span>
                @endif
            </div>
