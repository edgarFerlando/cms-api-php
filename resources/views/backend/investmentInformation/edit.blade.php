...
<div class="content">
	<div class="box">  
		{!! Form::open( array( 'route' => array( getLang() . '.admin.investment-information.update', $item->id), 'method' => 'PATCH', 'files'=>true)) !!}
		...
		<div class="box-body">
			<div class="form-group {!! $errors->has('product_name') ? 'has-error' : '' !!}">
				...
				{!! 
					Form::text('product_name', val($item, 'product_name'), [
						'class'			=> 'form-control', 
						'autocomplete' 	=> 'off', 
					]) 
				!!}
			...
			<div class="form-group {!! $errors->has('description') ? 'has-error' : '' !!}">
				...
				{!! 
					Form::textarea('description', val($item, 'description'), [
						'class'=>'form-control',
						'rows'=> 3
					]) 
				!!}
				...
				<div class="row">
					...
					<div class="col-lg-3">
						<div class="form-group {!! $errors->has('scoring_3_thn') ? 'has-error' : '' !!}">
							...
							{!! 
								Form::select('scoring_3_thn', $stars_options, val($item, 'scoring_3_thn'), [
									'class'			=> 'form-control', 
									'autocomplete' 	=> 'off', 
								]) 
							!!}
					...
					<div class="col-lg-3">
						<div class="form-group {!! $errors->has('inv_1_hr') ? 'has-error' : '' !!}">
							...
							{!! 
								Form::text('inv_1_hr', val($item, 'inv_1_hr'), [
									'class'			=> 'form-control number', 
									'autocomplete' 	=> 'off', 
								]) 
							!!}
					...
					<div class="col-lg-3">
						<div class="form-group {!! $errors->has('inv_1_bln') ? 'has-error' : '' !!}">
							...
							{!! 
								Form::text('inv_1_bln', val($item, 'inv_1_bln'), [
									'class'			=> 'form-control number', 
									'autocomplete' 	=> 'off', 
								]) 
							!!}
				...
				<div class="row">
					<div class="col-lg-3">
						<div class="form-group {!! $errors->has('inv_1_thn') ? 'has-error' : '' !!}">
							...
							{!! 	
								Form::text('inv_1_thn', val($item, 'inv_1_thn'), [
									'class'			=> 'form-control number', 
									'autocomplete' 	=> 'off', 
								]) 
							!!}
					...
					<div class="col-lg-3">
						<div class="form-group {!! $errors->has('inv_3_thn') ? 'has-error' : '' !!}">
							...
							{!! 
								Form::text('inv_3_thn', val($item, 'inv_3_thn'), [
									'class'			=> 'form-control number', 
									'autocomplete' 	=> 'off', 
								]) 
							!!}
					...
					<div class="col-lg-3">
						<div class="form-group {!! $errors->has('since_launched') ? 'has-error' : '' !!}">
							...
							{!! 
								Form::text('since_launched', val($item, 'since_launched'), [
									'class'			=> 'form-control number', 
									'autocomplete' 	=> 'off', 
								]) 
							!!}
					...
					<div class="col-lg-3">
						<div class="form-group {!! $errors->has('fluctuation') ? 'has-error' : '' !!}">
							...
							{!! 
								Form::text('fluctuation', val($item, 'fluctuation'), [
									'class'			=> 'form-control number', 
									'autocomplete' 	=> 'off', 
								]) 
							!!}
							...
							<table class="table table-bordered table-condensed table-white table-striped conrows">
								...
								<tbody>
									<?php
										...
										if (is_null($provider_datarow) && isset($item)) {
											$provider_datarow = [];
											foreach($item->providers as $key => $provider){
												$provider_datarow[$key]['provider_id'] = [
													'val' => $provider['id'],
                              						'text' => $provider['id']
												];
												$provider_datarow[$key]['provider_name'] = [
													'val' => $provider['provider_name'],
                              						'text' => $provider['provider_name']
												];
												$provider_datarow[$key]['pic_name'] = [
													'val' => $provider['pic_name'],
                              						'text' => $provider['pic_name']
												];  
												$provider_datarow[$key]['pic_email'] = [
													'val' => $provider['pic_email'],
                              						'text' => $provider['pic_email']
												]; 
											}
											
											$idx = 0; 
											foreach ($provider_datarow as $provider) { 
												$hidden_data = mapping_fieldType('provider', $provider);
												echo '<tr>';
												echo '<td>' . $provider['provider_name']['val'] . '</td>';
												echo '<td>' . $provider['pic_name']['val'] . '</td>';
