...
use App\Repositories\TriangleLayer\TriangleLayerInterface;
use Redirect;
use View;
use Input;
use Validator;
...
class TriangleLayerController extends Controller {
    protected $user;
    protected $triangleLayer;

    public function __construct(UserInterface $user, TriangleLayerInterface $triangleLayer) {
        $this->user = $user;
	...
	public function index()
	{
        ...
        $filter = Input::all(); 
        unset($filter['_token']); 
        $pagiData = $this->triangleLayer->paginate($page, $perPage, $filter);
		...
		return view('backend.triangleLayer.index', compact('items', 'totalItems'));
	}
	...
	public function create()
	{
        ...
        return view('backend.triangleLayer.create');
...
/*		$attr = [ 
	...
	public function store()
	{
		try {
            ...
            //dd($input);
            $this->triangleLayer->create($input);
            Notification::success( trans('app.triangle_layer_added') );
            return langRedirectRoute('admin.settings.plan-analysis.triangle-layer.index');
        } catch (ValidationException $e) { //dd($e->getErrors());
            return langRedirectRoute('admin.settings.plan-analysis.triangle-layer.create')->withInput()->withErrors($e->getErrors());
        }
	}
	...
	{   
        ...
        $data = $this->triangleLayer->find($id);
        return view('backend.triangleLayer.edit', compact('data'));
	}
	...
	{  
		try {   
            $input = Input::all();
            $this->triangleLayer->update($id, $input);
            Notification::success(trans('app.triangle_layer_updated'));
            return langRedirectRoute('admin.settings.plan-analysis.triangle-layer.index');
        } catch (ValidationException $e) {
            return langRedirectRoute('admin.settings.plan-analysis.triangle-layer.edit', [ 'id' => $id ] )->withInput()->withErrors($e->getErrors());
        }
	}
    ...
    public function confirmDestroy($id) {
        ...
        $triangle = $this->triangleLayer->find($id);
        //$user = $this->user->find($CfpClient->client_id);
        //dd($user);
        return view('backend.triangleLayer.confirm-destroy', compact('triangle'));
    }
