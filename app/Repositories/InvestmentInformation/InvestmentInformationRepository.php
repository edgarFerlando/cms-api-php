<?php namespace App\Repositories\InvestmentInformation;

use App\Models\InvestmentInformation;
use App\User;
use Config;
use Response;
//use App\Tag;
...
class InvestmentInformationRepository extends RepositoryAbstract implements InvestmentInformationInterface, CrudableInterface {
...
*/
public function __construct(InvestmentInformation $investmentInformation) {
  $this->investmentInformation = $investmentInformation;
  $this->investmentInformationProvider = new InvestmentInformationProvider;
...
public function rules(){
    ...
    $_rules['product_name'] = 'required';
    $_rules['description'] = 'required';
    $_rules['nab'] = 'required|numeric';
    $_rules['scoring_3_thn'] = 'required';
    $_rules['inv_1_hr'] = 'required|numeric';
    $_rules['inv_1_bln'] = 'required|numeric';
    $_rules['inv_1_thn'] = 'required|numeric';
    $_rules['inv_3_thn'] = 'required|numeric';
    $_rules['since_launched'] = 'required|numeric';
    $_rules['fluctuation'] = 'required|numeric';

    $setAttributeNames['product_name'] = trans('app.product_name');
    $setAttributeNames['description'] = trans('app.description');
    $setAttributeNames['nab'] = trans('app.nab');
    $setAttributeNames['scoring_3_thn'] = trans('app.scoring_3_thn');
    $setAttributeNames['inv_1_hr'] = trans('app.1_hr');
    $setAttributeNames['inv_1_bln'] = trans('app.1_bln');
    $setAttributeNames['inv_1_thn'] = trans('app.1_thn');
    $setAttributeNames['inv_3_thn'] = trans('app.3_thn');
    $setAttributeNames['since_launched'] = trans('app.since_launched');
    $setAttributeNames['fluctuation'] = trans('app.fluctuation');
    return [
        'rules' => $_rules,
...
public function all() {
  ...
  return $this->investmentInformation->with('providers')->orderBy('product_name', 'DESC')->get();
}
...
public function paginate($page = 1, $limit = 10, $filter = array()){//$all = false) {
    ...
    $query = $this->investmentInformation->with(['providers'])->orderBy('created_at', 'DESC');
...
public function find($id) {
  return $this->investmentInformation->with('providers')->find($id);
...
function create($attributes){
    ...
    if($this->isValid($attributes)) { //dd($attributes);
        ...
        $nab = $attributes['nab'];
        $scoring_3_thn = $attributes['scoring_3_thn'];
        $inv_1_hr = $attributes['inv_1_hr'];
        $inv_1_bln = $attributes['inv_1_bln'];
        $inv_1_thn = $attributes['inv_1_thn'];
        $inv_3_thn = $attributes['inv_3_thn'];
        $since_launched = $attributes['since_launched'];
        $fluctuation = $attributes['fluctuation'];
        $t_attributes = [ 
            'product_name' => $product_name,
            'description' => $description,
            'nab' => $nab,
            'scoring_3_thn' => $scoring_3_thn,
            'inv_1_hr' => $inv_1_hr,
            'inv_1_bln' => $inv_1_bln,
            'inv_1_thn' => $inv_1_thn,
            'inv_3_thn' => $inv_3_thn,
            'since_launched' => $since_launched,
        ...
        $investment_information = $this->investmentInformation->create($t_attributes);
        $investment_information_id = $investment_information->id;
       //save providers
        if(isset($attributes['provider_datarow'])){
            $providers_datarow = buildPOST_fromJS($attributes['provider_datarow']);
            foreach($providers_datarow as $datarow){
                $providers[] = [
                    'investment_information_id' => $investment_information_id,
                    'provider_name' => $datarow['provider_name']['val'],
                    'pic_name' => $datarow['pic_name']['val'],
                    'pic_email' => $datarow['pic_email']['val']
                ];
            }
            $this->investmentInformationProvider->insert($providers);
        }
...
public function update($id, $attributes) {
    if($this->isValid($attributes)) { //dd($attributes);
        DB::beginTransaction(); 
        $user_id = Auth::user()->id;
        $this->investmentInformation = $this->find($id);
        $product_name = $attributes['product_name'];
        $description = $attributes['description'];
        $nab = $attributes['nab'];
        $scoring_3_thn = $attributes['scoring_3_thn'];
        ...
        $investmentInformation = $this->investmentInformation->update($t_attributes);

        //save investment information
        //++$this->investmentInformation->providers()->delete($id);
        $provider = [];
        $providers = [];
        $still_exist_provider_id = [];
        if(isset($attributes['provider_datarow'])){
            
            $provider_datarow = buildPOST_fromJS($attributes['provider_datarow']); //dd($provider_datarow);
            foreach($provider_datarow as $datarow){
                foreach($datarow as $ff_name => $ff){
                    $provider[$ff_name] = $ff['val'];
                    if($ff_name == 'provider_id' && $provider[$ff_name] != '')
                        $still_exist_provider_id[] = $provider[$ff_name];
                }
                $providers[] = [
                    'investment_information_id' => $id,
                    'id' => isset($datarow['provider_id'])?$datarow['provider_id']['val']:'',
                    'provider_name' => $datarow['provider_name']['val'],
                    'pic_name' => $datarow['pic_name']['val'],
                    'pic_email' => $datarow['pic_email']['val']
                ];
        ...
        //dd($providers);
        //dd($still_exist_provider_id);
        //$this->investmentInformation->providers()->where('investment_information_id', $id)->update(['is_deleted' => 1 ]);
        $applied_providers_raw = $this->investmentInformation->providers()->where('investment_information_id', $id)->whereNotIn('id', $still_exist_provider_id)->get();
        $remove_providers = [];
        foreach($applied_providers_raw as $applied_provider){
            $remove_providers[] = $applied_provider->id;
        }
        //dd($remove_providers);
        //dd($providers);
        if(count($providers)){
            foreach($providers as $provider_item){ //dd($provider_item);
                $provider_item_safe = $provider_item;
                unset($provider_item_safe['id']);
                if(isset($provider_item['id']) && $provider_item['id'] != '' ){
                    $provider_exist = InvestmentInformationProvider::where([ 
                        'investment_information_id' => $provider_item['investment_information_id'], 
                        'id' => $provider_item['id'] ])->first();
                    if(!is_null($provider_exist)){
                       // $variant_attributes['id'] = $prodVariation->id;
                        $provider_exist->update($provider_item_safe);
                        //$provider_exist->save();
                    }else{
                        InvestmentInformationProvider::insert($provider_item_safe);
                    }
                }else{ //dd($provider_item_safe);
                    InvestmentInformationProvider::insert($provider_item_safe);
                }
            }
        }
        InvestmentInformationProvider::whereIn('id', $remove_providers)->delete();
        ...
        //$this->investmentInformation->providers()->where('is_deleted', 1)->delete();
...
public function delete($id) {
  ...
  $this->investmentInformation = $this->investmentInformation->find($id);
  $this->investmentInformation->providers()->delete($id);
  $this->investmentInformation->delete();
}
...
protected function totalProducts($filter = array()) {
  $query = $this->investmentInformation->select('id');
   //if(!$all) {
