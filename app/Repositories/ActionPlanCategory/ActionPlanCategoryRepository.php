<?php namespace App\Repositories\ActionPlanCategory;
...
class ActionPlanCategoryRepository extends RepositoryAbstract implements ActionPlanCategoryInterface, CrudableInterface {
     ...
     */
    public function __construct(ActionPlanCategory $triangleLayer) {

        $config = Config::get('holiday');
        $this->perPage = $config['per_page'];
        $this->actionPlanCategory = $triangleLayer;
    ...
    public function all() {
        return $this->actionPlanCategory->orderBy('created_by', 'DESC')->get();
    }
    ...
    public function paginate($page = 1, $limit = 10, $filter = array()) {
        ...
        $query = $this->actionPlanCategory->select('action_plan_categories.*', 'uc.name as created_by_name', 'uu.name as updated_by_name');
        
        //->join('taxonomies as t', 't.id', '=', 'ActionPlanCategory.step_2', 'left');
        //->with('layer');
        $query->orderBy('action_plan_categories.created_at', 'DESC');
        $query->join('users as uc', 'uc.id', '=', 'action_plan_categories.created_by', 'left');
        $query->join('users as uu', 'uu.id', '=', 'action_plan_categories.updated_by', 'left');
                ...
                if(trim($term) != ''){
                    switch ($ff) {
                        case 'record_flag_is_not':
                            $query->where('ActionPlanCategory.record_flag', '!=', $term);
                        break;
                        case 'client_name':
        ...
        $ActionPlanCategorys = $query->skip($limit * ($page - 1))->take($limit)->get();

        //dd(\DB::getQueryLog());
        //dd($categoryCodes);

        $result->totalItems = $this->totalActionPlanCategorys($filter);
        $result->items = $ActionPlanCategorys->all();
/*
        foreach ($result->items as $key => $item) {
            $result->items[$key]->ActionPlanCategorys = $item;
    ...
    public function find($id) {
        ...
        ->select('action_plan_categories.*','uc.name as created_by_name', 'uu.name as updated_by_name')
        ->join('users as uc', 'uc.id', '=', 'action_plan_categories.created_by', 'left')
        ->join('users as uu', 'uu.id', '=', 'action_plan_categories.updated_by', 'left')
        ->find($id);
    }
    ...
    public function findAllBy($attributes) { //sementara dikosongin
        ...
        //$this->actionPlanCategory->whereBetween('schedule_start_date', [$start_date, $end_date])->get()->toArray();
        //dd(\DB::getQueryLog());
...
/*
    ...
    public function create($attributes) {
        if($this->isValid($attributes)) {
            ...
            $this->actionPlanCategory->create([
        ...
        throw new ValidationException('Action Plan Category layer validation failed', $this->getErrors());
    }
    ...
    public function update($id, $attributes) {
        if($this->isValid($attributes)) {
            ...
            $this->actionPlanCategory->where('id', $id)->update([
                'title' => $attributes['title'],
                'updated_by' => $user_id, 
                'updated_at' => Carbon::now()
            ]);
    ...
    public function delete($id) {
        ...
        $ActionPlanCategory = $this->actionPlanCategory->find($id);
        $ActionPlanCategory->delete();
    }

    public function api_delete($id) {
        $ActionPlanCategory = $this->actionPlanCategory->findOrFail($id);
        if($ActionPlanCategory){
            $ActionPlanCategory->delete();
            return true;
        }
        return false;
    ...
    protected function totalActionPlanCategorysJUNK($all = false) {
        return $this->actionPlanCategory->count();
    }

    protected function totalActionPlanCategorys($filter = array()) {
        $query = $this->actionPlanCategory
    ...
    public function availableTimeSlot($filter = array()){//$attributes){
        $query = $this->actionPlanCategory->orderBy('created_at', 'DESC');
        if(is_array($filter)){
            foreach($filter as $ff => $term){
                if(trim($term) != ''){
                    switch ($ff) {
