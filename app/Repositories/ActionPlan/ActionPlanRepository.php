<?php namespace App\Repositories\ActionPlan;
...
class ActionPlanRepository extends RepositoryAbstract implements ActionPlanInterface, CrudableInterface {
     ...
     */
    public function __construct(ActionPlan $triangleLayer) {
    ...
    public function paginate($page = 1, $limit = 10, $filter = array()) {
        ...
        //->join('taxonomies as t', 't.id', '=', 'ActionPlan.step_2', 'left');
        //->with('layer');
                ...
                if(trim($term) != ''){
                    switch ($ff) {
                        case 'record_flag_is_not':
                            $query->where('ActionPlan.record_flag', '!=', $term);
                        break;
                        case 'client_name':
        ...
        $ActionPlans = $query->skip($limit * ($page - 1))->take($limit)->get();

        //dd(\DB::getQueryLog());
        //dd($categoryCodes);

        $result->totalItems = $this->totalActionPlans($filter);
        $result->items = $ActionPlans->all();
/*
        foreach ($result->items as $key => $item) {
            $result->items[$key]->ActionPlans = $item;
...
/*
    ...
    public function create($attributes) {
        ...
        throw new ValidationException('ActionPlan layer validation failed', $this->getErrors());
    }
    ...
    public function delete($id) {
        ...
        $ActionPlan = $this->triangleLayer->find($id);
        $ActionPlan->delete();
    }

    public function api_delete($id) {
        $ActionPlan = $this->triangleLayer->findOrFail($id);
        if($ActionPlan){
            $ActionPlan->delete();
            return true;
        }
    ...
    protected function totalActionPlansJUNK($all = false) {
        return $this->triangleLayer->count();
    }

    protected function totalActionPlans($filter = array()) {
